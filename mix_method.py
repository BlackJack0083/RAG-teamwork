...
def get_vectorstore(documents, embeddings, persist_directory):
    ...
    if os.path.exists(persist_directory) and os.listdir(persist_directory):
        ...
        # It's good practice to call persist() explicitly if you're not relying on it automatically
        # vectorstore.persist() # Chroma.from_documents with persist_directory usually saves it.
    return vectorstore
...
# 1. 创建 BM25 检索器
bm25_retriever = BM25Retriever.from_documents(documents_to_index, k=1)
...
ensemble_retriever = EnsembleRetriever(
    ...
    weights=[0.5, 0.5]  # 例如，各占 50% 的权重
)

# 使用 ensemble_retriever 替换原来的 retriever
...
template = """基于以下已知信息，请专业地回答用户的问题。
              ...
              answer1: 操作多媒体娱乐功能时，确保将车辆停驻在安全地点，将挡位切换至驻车挡（P）并使用驻车制动
...
def batch_answer_and_compare(questions_file, output_file):
    ...
    for question in questions:
        ...
        # When using EnsembleRetriever, 'context' might contain documents from both retrievers.
        # You might want to adjust how you extract the 'llm_reference' if you need
        # more nuanced information about which retriever provided which document.
        # For simplicity here, we'll just take the page number of the first document.
...
if __name__ == '__main__':
    batch_answer_and_compare("./QA_pairs.json", "./QA_pairs_answers_mix_method.json")